import pygame
from tiles import StaticTile, Crate, Coin, Palm, BGPalm, Tile
...
class Level:
    def __init__(self, current_level, surface, create_overworld, change_coins, change_health):
        ...
        self.stomp_sound.set_volume(0.01)
        ...
        #background palms
        bgpalm_layout = import_csv_layout(level_data['bg_palms'])
        self.bgpalm_sprites = self.create_tile_group(bgpalm_layout, 'bg_palms')
        ...
        #decoration
        self.sky = Sky(8)
        level_width = len(terrain_layout[1]) * tile_size
        self.water = Water(HEIGHT - 25, level_width)
        ...
        for row_index, row in enumerate(layout):
            for col_index, val in enumerate(row):
                if val != '-1':
                    ...
                    if type == 'bg_palms':
                        sprite = BGPalm(tile_size, x, y, 'graphics/terrain/palm_bg', 64)
    ...
    def hmove_collision(self):
        player = self.player.sprite
        player.collision_rect.x += player.direction.x * player.speed
        
        collidable_sprites = self.terrain_sprites.sprites() + self.crate_sprites.sprites() + self.palm_sprites.sprites()
    ...
    def run(self):
        ...
        #player
        self.player.update()
        self.hmove_collision()
        self.get_player_on_ground()
        self.vmove_collision()
